...
if (is_post() && isset($_POST['request_otp'])) {
    ...
    try {
        ...
        $stmt = $_db->prepare("SELECT * FROM customers WHERE email = ?");
        $stmt->execute([$email]);
...
if (is_post()) {
    ...
    if (empty($otpEntered)) {
        ...
        $stmt = $_db->prepare("UPDATE customers SET password = ? WHERE email = ?");
    ...
    if ($msg) {
        ...
        if ($isSuccess) {
            ...
            redirect('login.php'); // Redirect on successful password reset
        }
    }
}
?>
    ...
    <div id="container">
        ...
        <div id="container-right">
            <form id="forgot-password-container" action="" method="POST">
                ...
                <button id="reset-password-btn" type="submit" class="btn btn-reset">Reset Password</button>
            </form>
        </div>
    </div>
